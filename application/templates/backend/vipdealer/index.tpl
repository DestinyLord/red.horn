						...
						<div class="portlet box light-grey">
							...
							<div class="portlet-body">
								...
								<div class="clearfix">
                                    ...
                                    <div class="btn-group">
                                        <{if check_priv('VIPDealerGrouponAdd')}>
                                        <a href="<{site_url uri=$backendDir|cat:'/vipdealer/addGroup'}>" id="sample_editable_1_new" class="btn green">

                                        添加团购 <i class="icon-plus"></i>
								...
								<table class="table table-striped table-bordered table-hover" id="sample_4">
									...
									<thead>
										...
										<tr>
											...
											<th><a href="<{current_url()}>?order_by=dealer_id">经销商</a></th>
                                            
											<th>活动类型</th>

											<th>活动车型</th>

                                            <th><a href="<{current_url()}>?order_by=add_time">活动发布时间</a></th>

                                            <th><a href="<{current_url()}>?order_by=end_time">活动结束时间</a></th>
                                            
                                            <th><a href="<{current_url()}>?order_by=status">审核状态</a></th>
									...
									<tbody>
										...
										<tr class="odd gradeX">
											...
											<td><{if $v.type == 1}>特价车<{else}>团购<{/if}></td>

											<td><{$v.series_id|showSeriesNameList:$v.model_name}></td>
                                            
                                            <td><{date("Y-m-d ", $v.add_time)}></td>

                                            <td><{date('Y-m-d',$v.end_time)}></td>
                                            
                                            <td><{if $v.status==1}>审核通过
                                                <{elseif $v.status==0}>未审核
                                                <{elseif $v.status == -1}>审核未通过，原因：<{$v.check_reason}>
                                                <{/if}></td>
                                            
                                            <td>
                                            	<{if check_priv('VIPDealerGrouponEdit')}>
                                                <a href="<{if $v.type == 1}><{site_url uri=$backendDir|cat:'/vipdealer/editCar/'|cat:$v.id}><{else}><{site_url uri=$backendDir|cat:'/vipdealer/editGroup/'|cat:$v.id}><{/if}>">编辑</a>
                                                <{/if}>
                                                &nbsp;&nbsp;
                                                <a target="blank" href="http://df.shopmodule.jaeapp.com/index.php/active/groupBuyingPreview/<{$v.id}>">预览</a>
                                                <{if $v.status==1 && $v.end_time > time()}>
												&nbsp;&nbsp;
                                                <img src="/uploads/images/QRCode/vipdealerGroup/<{$v.id}>.png" width="80" height="80" />
                                                <{/if}>
                                            </td>
