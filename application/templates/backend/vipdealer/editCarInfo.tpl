								...
								<form id="dataform" action="<{$formAction}>" method="post" class="form-horizontal form-bordered info-form">
									...
									<div class="control-group">
										...
										<div class="controls">
											...
											<div class="select2-wrapper">
												...
												<select name="series_id" class="span4 select2_category">

													<option value="">请选择车系</option>
													<{if check_priv('VIPDealerCarCheckOnline')}>
														<optgroup label="日产">
															<{foreach from=$seriesList1 item=ns}>
															<option <{if $ns.series_id == $info.series_id}>selected="true"<{/if}> value="<{$ns.series_id}>"><{$ns.name}></option>
														...
														<optgroup label="启辰">
															...
															<option <{if $qcs.series_id == $info.series_id}>selected="true"<{/if}> value="<{$qcs.series_id}>"><{$qcs.name}></option>
															<{/foreach}>
														</optgroup>
												...
												<select name="model_id" class="span4 select2_category" dataType="Require" msg="车型必须选择">
													<option value="0">请选择</option>
													<{foreach from=$modelList item=ml}>
													<option <{if $info.model_id == $ml.model_id}>selected="true"<{/if}> value="<{$ml.model_id}>"><{$ml.model_name}></option>
													<{/foreach}>
												</select>
											...
											<div class="select2-wrapper">
												...
												<select name="color" class="span6 select2_category" dataType="Require" msg="车型颜色必须选择">
													...
													<option value="">请选择颜色</option>
                                                  	<{foreach from=$modelColor item=v}>
                                                  	<option <{if $info.color == $v.color_name}>selected="true"<{/if}> value="<{$v.color_name}>"><{$v.color_name}></option>
                                                  	<{/foreach}>
                                    ...
                                    <div class="control-group">
										...
										<div class="controls">
											...
											<input dataType="Number" msg="库存必须为正整数" required="true" id="stock" name="stock" value="<{$info.stock}>" type="text" placeholder="库存" class="m-wrap span6" />
                                    ...
                                    <div class="control-group ">
										...
										<div class="controls">
											...
											<input dataType="BetweenTo" require="true" msg="价格必须为大于0的整数且不能大于车型指导价" min="1" max="<{$modelInfo.model_price/100}>" required="true" id="price" name="price" value="<{$info.price}>" type="text" placeholder="价格" class="m-wrap span6" /> 
											<span class="help-inline">元 官方指导价：<font class="model-price"><{$modelInfo.model_price/100}></font>元，优惠：<font class="favorable-price"><{$modelInfo.model_price/100 - $info.price}></font>元</span>
                                    ...
                                    <div class="control-group policyBox">
										...
										<div class="controls">
											<{if $info.policy}>
												<{foreach $info.policy item=v name="poli"}>
												<{if $smarty.foreach.poli.index != 0}><div class="clearfix"></div><br/><{/if}>
												<input <{if $smarty.foreach.poli.index == 0}>id="policy"<{/if}> name="policy[]" value="<{$v}>" type="text" placeholder="优惠政策10个字以内" class="m-wrap span6" />
									...
									<div class="control-group <{if !check_priv('VIPDealerCarCheckOnline')}>last<{/if}>">
										...
										<div class="controls">
											...
											<div class="input-append date date-picker">
												...
												<input dataType="Require" msg="活动时间必须填写" class="m-wrap m-ctrl-medium date-picker" id="end_time" name="end_time" readonly size="16" type="text" value="<{$info.end_time|date_format:'%Y-%m-%d'}>" /><span class="add-on" ><i class="icon-calendar"></i></span>
									...
									<div class="control-group last">
										...
										<div class="controls">
												...
												<select name="status" id="status" class="span2" style="margin: 2px 0;">
													...
													<option <{if $info.status == 1}>selected="true"<{/if}> value="1">审核通过</option>
													<option <{if $info.status == -1}>selected="true"<{/if}> value="-1">审核不通过</option>
												</select>
												<input id="check_reason" name="check_reason" value="<{$info.check_reason}>" type="text" placeholder="审核未通过请给出原因" class="m-wrap span4"  style="margin-left:10px;"/>
            	...
            	$.post(
            		...
            		function(res)
            		{
            			if(res != '')
            			{
            				...
            				if(priceVal != '')
            				{
            					$('.favorable-price').text(res.model_price / 100 - priceVal);
            				}
