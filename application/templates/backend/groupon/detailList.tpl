						...
						<div class="portlet-body">
							<div class="clearfix">
                                ...
                                <div class="btn-group">
                                    ...
                                    <a href="<{site_url uri=$backendDir|cat:'/groupon/index'}>" id="sample_editable_1_new" class="btn green">

                                    我要报名 <i class="icon-plus"></i>
                                ...
                                <div class="btn-group pull-right">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        ...
                                        <li><a href="<{site_url uri=$backendDir|cat:'/groupon/detailList'}>?export=1">导出到EXCEL</a></li>
							...
							<table class="table table-striped table-bordered table-hover" id="sample_4">
								...
								<thead>
									...
									<tr>
										...
										<th><a href="<{current_url()}>?order_by=add_time">报名时间</a></th>

										<th>参团车型</th>

                                        <th>开团时间</th>

                                        <th><a href="<{current_url()}>?order_by=status">审核状态</th>
								...
								<tbody>
									...
									<tr class="odd gradeX">
										...
										<td><{$v.series_id|showSeriesNameList}></td>
                                        
                                        <td><{implode(',' ,json_decode($v.join_date))}></td>
                                        
                                        <td>
                                        	<{if $v.status==1}>未处理
                                            <{elseif $v.status==2}>审核通过
                                            <{elseif $v.status == 3}>审核未通过，原因：<{$v.check_reason}>
                                            <{/if}>
                                        </td>
                                        
                                        <td>
                                            <a href="<{site_url uri=$backendDir|cat:'/groupon/detailEdit/'|cat:$v.id}>">编辑</a>
                                            <{assign var='seriesArr' value=$v.series_id|json_decode}>
                                            <{if $v.status != 3}>
                                            &nbsp;&nbsp;<a target="_blank" href="https://df.shopmodule.jaeapp.com/index.php/dealerActive/groupDetail<{if $v.status != 2}>Preview<{/if}>/<{$seriesArr.0}>/<{$v.dealer_id}>?city_id=<{$v.dealer_city}>">PC预览</a>
                                            &nbsp;&nbsp;
                                            <img src="/uploads/images/QRCode/grouponDetail<{if $v.status != 2}>Preview<{/if}>/<{$v.id}>.png" width="80" height="80" />
                                            <{/if}>
                                            <!--&nbsp;&nbsp;
                                            <{if check_priv('grouponDetailDel')}>
